...
from models.sl.gcn_layers import ResidualGatedGCNLayer, MLP
...
class ResidualGatedGCNModelForCND(nn.Module):
    def __init__(self, 
            node_dim:int, # = 4, 5
            hidden_dim:int = 64,
            num_layers:int = 3,
        ...
        super(ResidualGatedGCNModelForCND, self).__init__()
        # Define net parameters
        ...
        gcn_layers = []
        for layer in range(self.num_layers):
            gcn_layers.append(ResidualGatedGCNLayer(self.hidden_dim, dropout, self.aggregation))
        self.gcn_layers = nn.ModuleList(gcn_layers)
        # Define MLP classifiers
        self.mlp_nodes = MLP(self.hidden_dim, 1, self.mlp_layers)
    ...
    def forward(self, 
        ...
        """
        Args:
            X: node vectors (B, V, 4 or 5)
            A: adjacency matrix (B, V, V, 1)
        ...
        """
        # Node and edge embedding
        x = self.nodes_coord_embedding(X)  # B x V x H
        e = self.edges_values_embedding(A.unsqueeze(3))  # B x V x V x H
        # GCN layers
        for layer in range(self.num_layers):
            x, e = self.gcn_layers[layer](x, e)  # B x V x H, B x V x V x H
        # MLP classifier
        y_pred_nodes = self.mlp_nodes(x)  # B x V x voc_edges_out
        
        return torch.sigmoid(y_pred_nodes)